...
class TestRactor < Test::Unit::TestCase
  ...
  test "decode using Ractor" do
    r = Ractor.new(TEST_DATA) { |data|
      JPEG::Decoder.new << data
    }
  ...
  test "encode using Ractor" do
    dec = JPEG::Decoder.new(:pixel_format => :YCbCr)

    img = assert_nothing_raised {dec << TEST_DATA}

    r = Ractor.new(img) { |data|
      met = data.meta
      enc = JPEG::Encoder.new(met.width,
                              met.height,
                              :pixel_format => :YCbCr)
      enc << data
    }

    dat = assert_nothing_raised {r.take}
  ...
  test "using decode result in Ractor" do
    assert_nothing_raised {
      r = Ractor.new(JPEG::Decoder.new << TEST_DATA) { |raw|
        raw.meta
      }

      assert_true(r.take.frozen?)
    }
  end

  test "using encode result in Ractor" do
    raw = JPEG::Decoder.new << TEST_DATA
    met = raw.meta
    enc = JPEG::Encoder.new(met.width,
                            met.height,
                            :pixel_format => met.output_colorspace)

    assert_nothing_raised {
      r = Ractor.new(enc << raw) {|jpg| jpg.bytesize}
      r.take
    }
  end
end
